#!/usr/bin/env python3
"""Run prevent_breakage across several seeds and print/save a results report.
...
Usage (robodyna env, headless Vulkan)::
    ...
    python -u script/bench_script/eval_prevent_breakage.py
    python -u script/bench_script/eval_prevent_breakage.py --seeds 11,23,37,41,59
"""
...
def _run_worker(seed: int, out_json: Path) -> int:
    ...
    import numpy as np
    from envs.prevent_breakage import prevent_breakage
    from record_demo import build_args

    args = build_args(
        "prevent_breakage", "demo_dynamic",
...
def _spawn_seed(seed: int, out_dir: Path) -> dict:
    ...
    cmd = [
        sys.executable, "-u",
        "script/bench_script/eval_prevent_breakage.py",
        "--worker", str(seed),
...
def main():
    ...
    parser.add_argument("--out", default="tmp_prevent_breakage/eval")
