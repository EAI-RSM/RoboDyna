#!/usr/bin/env python3
"""One-shot dual-view recorder for fixed seeds (no infinite retry loop)."""
...
def record_seed(task_name: str, seed: int, config_name: str = "demo_dynamic") -> dict:
    ...
    print(f"[plan] {task_name} seed={seed}", flush=True)
    ...
    if not ok:
        task.close_env()
        raise RuntimeError(f"plan pass failed for {task_name} seed {seed}")
    ...
    print(f"[render] {task_name} seed={seed}", flush=True)
    ...
    final_dir = os.path.abspath(f"./docs/final_task_demos/{task_name}/eval")
    ...
    print(f"Demo {task_name} seed={seed}:")
    print("  head     :", out_head)
    print("  top-down :", out_topdown)
    print("  side-by-side:", out_side)
    return {
        "task": task_name, "seed": seed, "version": ver, "ok": True,
...
def main():
    ...
    for seed in seeds:
        print(f"\n######## recording {ns.task} seed {seed} ########", flush=True)
        try:
            record_seed(ns.task, seed, ns.config)
