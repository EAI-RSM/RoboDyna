...
- `script/_install.sh` now has `set -e` (fixed 2026-07-07, see TECHNICAL_DEBT_AUDIT.md INST-1) and fails fast on `pip install`, `pip show`/`sed` patch, and curobo-clone failures, and hard-exits if the post-retry pytorch3d/curobo import checks fail. Its exit code can now be trusted. (Source: script/_install.sh:2,39,73-74,98-99,119)
