...
class Base_Task(gym.Env):
    ...
    def mark_phase(self, label, **meta):
        """Record the start of an action/skill segment at the current saved-frame index, for the
        LeRobot annotation layer. Optional; tasks may call this in play_once to tag primitives
    ...
    def left_move_to_pose(
        ...
        if left_result["status"] != "Success":
            self.plan_success = False
            self._last_plan_fail = ("left", left_result.get("reason", "?"))
            return
    ...
    def right_move_to_pose(
        ...
        if right_result["status"] != "Success":
            self.plan_success = False
            self._last_plan_fail = ("right", right_result.get("reason", "?"))
            return
