try:
    ...
    _HAS_MPLIB = True
except Exception:  # mplib has no working aarch64 build; CuroboPlanner is used instead
    mplib = None
    SapienPlanner = SapienPlanningWorld = None
    _HAS_MPLIB = False
import numpy as np
import pdb
import traceback
    ...
    class CuroboPlanner:
        ...
        def plan_path(
            ...
            if result.success.item() == False:
                ...
                try:
                    res_result["reason"] = str(result.status)   # curobo status: IK_FAIL / TRAJOPT_FAIL / ...
                except Exception:
                    res_result["reason"] = "unknown"
                return res_result
            else:
