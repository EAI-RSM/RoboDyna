...
|------|-------------|:------:|------|
| **cook_meat** | Grasp raw steak(s) from the cutting board, cook on pan (speed = nominal ±20%), then return to the board. **Default**: contact cook (no key). **Opt 1** (`cook_button_enabled`): hold red key to cook (key on the same side as the station). **Opt 2** (`dual_setup_enabled`): mirrored dual stations with ≥10 cm clearance; both arms place/pick together; **both steaks must be cooked properly**. **Opt 1+2**: dual stations each with a cook key (same both-steaks success rule). | ✅ **done** | <img src="assets/static/demos/cook_meat.gif" width="320"/> |
...
| **sort_apples_belt** | 4–10 apples stream down a conveyor; press the matching side button to aim a two-piece wooden diverter plank into the color-matched basket. **Default**: alternating red/green. **Opt 1**: random colors. **Opt 2**: with prob. ~30% one brown rotten apple — press both buttons to open the plank so it dumps into a garbage bin at the belt end. | ✅ **done** | <img src="assets/static/demos/sort_apples_belt.gif" width="320"/> |
| **pick_ripe_apple** | Default: one apple hangs and ripens green→**red**→black; matching arm drops it in a static basket. **Opt 1** (`two_apples_enabled`): second apple green→**yellow**→black (ignore). **Opt 2** (`basket_move_enabled`): basket oscillates under the branches. | ✅ **done** | <img src="assets/static/demos/pick_ripe_apple.gif" width="320"/> |
| **hit_target** | A target sways about table center (x=0); optional green static / red dynamic blockers; L/R stick; expert sticks the tip in the yellow center. | ✅ ready | <img src="assets/static/demos/hit_target.gif" width="320"/> |
...
| `whack_a_mole` | Whack-a-mole: 2 moles with randomized pop speeds; Opt1 rabbit distractor; Opt2 moles relocate to free holes when they go down (combinable). | <img src="assets/static/demos/whack_a_mole.gif" width="240"/> |
| `catch_rat` | Grasp a rat that pops up from holes on a mid-table board. Config: `catch_two_mice` (opt1) = two simultaneous mice, both must be held; `opaque_surface` (opt2) = solid colored top (default glass). | <img src="assets/static/demos/catch_rat.gif" width="240"/> |
| `catch_valley_ball` | Place a small bowl beyond the 10 cm red boundary to catch a red ball leaving a curved down-then-up ramp. Platform length varies ±20%. Layout randomly mirrors (left arm when mirrored). Default: straight exit. Opt 1: wall rebound. Opt 2: black distractor on a separate exit lane (catching it fails). | <img src="assets/static/demos/catch_valley_ball.gif" width="240"/> |
...
| `dispense_gummy` | Press the dispense key and belt arrow keys so only the target-colored gummies land in the bowl. Default: alternating target/distractor layouts + discrete belt hops. Opt1 `layout_mode=random` (≤1 target per depth; distractors may both appear). Opt2 `belt_continuous_motion` (hold arrow key; speed ±20%). | <img src="assets/static/demos/dispense_gummy.gif" width="240"/> |
...
| `load_train` | Pick up a ball and drop it into an open wagon on a continuously circling toy train (default: any of 3 wagons). Opt 1: one random red target wagon (others gray). Opt 2: arched tunnel on the far arc. | <img src="assets/static/demos/load_train.gif" width="240"/> |
